...
| MSP-CA007 | Block unknown device platforms | Disabled | User-agent condition is mutable; approve supported-platform policy first |
...
| MSP-CA301 | Restrict Exchange/SharePoint downloads on unmanaged browsers | Report-only | SharePoint and Exchange configuration |
...
|---|---|---|---|
| MSP-CA400 | Medium/high sign-in risk requires MFA every time | Report-only | Entra ID P2 or Entra Suite |
| MSP-CA401 | High user risk requires risk remediation | Report-only | Entra ID P2 or Entra Suite |
...
- CA102 adds a privileged-workstation requirement over CA302/CA303 and should be enabled only when all administrator platforms are managed.
