...
A production-focused Microsoft Entra Conditional Access suite for repeatable deployment through CIPP. It contains **30 policies and 16 supporting groups**. Every policy starts in **Report-only**.
...
|---|---:|---|
| `SHOOTHILL-CA-01-Core-Identity-and-External-Access-P1` | 12 | MFA, registration, risky authentication flows, guests, and trusted-location guardrails |
...
| `SHOOTHILL-CA-03-Identity-Protection-P2` | 2 | P2 licensing, SSPR, and risk-response operations |
| `SHOOTHILL-CA-04-Workload-Identity-Premium` | 2 | Workload ID Premium and service-principal location inventory |
| `SHOOTHILL-CA-05-Defender-and-Purview-Advanced` | 2 | Defender App Control plus Purview/HR/legal governance |
...
Config/
  ...
  Groups/              16 portable security-group templates
...
5. In **Available Conditional Access Templates**, assign the policies to the five package names in `Config/PolicyPackages.psd1`.
...
The validator checks counts, JSON parsing, Report-only state, group mappings, complete and non-overlapping package membership, directory-sync exclusion, Device Registration Service exclusion, authentication-transfer coverage, token-protection scope, preview-field absence, and retired approved-client-app controls.
