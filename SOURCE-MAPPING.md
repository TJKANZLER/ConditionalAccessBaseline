...
Tenant-specific resource IDs, country lists, IP ranges, Terms of Use documents, and permanent exceptions are not fabricated. Additional MAM-protected target resources enter through `Config/PolicyExtensions.psd1`; mobile client-app IDs do not. CA011 resolves the separately maintained `SHOOTHILL-CA-Allowed-Countries-Operator-Defined` tenant location by display name. The underlying values remain tenant configuration with explicit ownership.
