...
| Internal user sessions | Microsoft Conditional Access session-control guidance | 24-hour sign-in frequency and nonpersistent browser sessions as a tenant-tunable starting point |
...
| Mobile access | Microsoft Intune App Protection and compliance guidance | App Protection for Office 365 by default, an audited application-ID extension for third-party MAM apps, plus full compliance for the managed-mobile include group |
...
| Network boundary | Microsoft location-based CA guidance | Trusted-location guardrails for admins, registration, and MFA-exempt service accounts; ordinary-user country restriction exists only in a separate explicit-adoption package with its own named-location placeholder |
...
Tenant-specific application IDs, country lists, IP ranges, Terms of Use documents, and permanent exceptions are not fabricated. Third-party MAM IDs enter through `Config/PolicyExtensions.psd1`; CA011 resolves the separately maintained `SHOOTHILL-CA-Allowed-Countries-Operator-Defined` tenant location by display name. The underlying values remain tenant configuration with explicit ownership.
