...
| Deployment structure | Five non-overlapping standard packages, with country restriction staged Report-only inside Core until its readiness gate passes | Alex provides multiple P1/P2 policy-set variants | Strong for a standardized MSP service with geographic protection present by default but safely staged |
...
| Policy breadth | 35 production policies covering P1, Intune, P2, workload, Defender, Purview, high-value identities, location-aware internal sessions, and Core country restriction | Kenneth has 49 CA policies; Alex has a 59-policy library; Aollivierre includes 44 CA policies plus supporting Intune/location artifacts | Intentionally smaller, with tenant-specific country values kept outside the portable policy payload |
...
| Repository governance | Changelog, CI, contribution guidance, and private security-reporting guidance are present | Microsoft and Alex also publish explicit reuse terms | Licence choice and release governance still need owner decisions |
...
2. **It limits cross-tenant object debt without sharing unrelated bypasses.** Twenty purpose-specific groups cover the suite; the registration and administrator location exceptions remain separate because collapsing them widened access unnecessarily.
...
- Split ordinary internal-user session hardening between CA010's 14-day trusted-location interval and CA012's 24-hour untrusted-location interval, with nonpersistent browser sessions in both.
- Added CA011 to the Core package using a tenant-owned allowed-country location rather than `AllTrusted`; it remains Report-only until tenant readiness is proven.
...
- Added explicit high-value-user phishing-resistant MFA and compliant-browser controls following a real token-theft compromise.
- Corrected CA401's remediation grant against the current Graph contract and added semantic regression validation.
- Added controlled Windows App token-protection extensions and repository contribution/security guidance.
...
1. automate a post-deployment inventory that proves all 35 policies exist in Report-only with resolved group and location IDs;
...
5. select and publish an explicit licence;
